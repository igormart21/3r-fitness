...
Be extremely specific with angles and positions. This is for sculpting — precision matters above everything.`,
    ...
    const prompt = [
      ...
      `• Camera: macro lens, perpendicular front view, FULL pendant centered with generous empty space on ALL sides — top, bottom, left, right — at least 15% margin on every edge so the ENTIRE pendant fits within the frame with NO cropping`,
      `• The pendant must float COMPLETELY within the image — bail visible at top, feet/base visible at bottom, nothing cut off`,
    ...
    console.log("[IA] Etapa 2: gerando pingente com gpt-image-1 images.edit...");
